...
function getWelcomeText(): string[] {
  return [
    ...
    chalk.dim('  explore → hypothesis → evidence → decision'),
    chalk.dim('  candidate → archive → findings'),
    '',
    chalk.white('Quick start after setup:'),
    `  ${chalk.yellow('/mlspec-explore')}           ${chalk.dim('Explore failure modes')}`,
    `  ${chalk.yellow('/mlspec-propose-experiment')} ${chalk.dim('Write hypothesis')}`,
    `  ${chalk.yellow('/mlspec-run-evidence')}      ${chalk.dim('Run E1/E2 evidence')}`,
    `  ${chalk.yellow('/mlspec-decide')}            ${chalk.dim('Promote/reject/hold')}`,
    '',
