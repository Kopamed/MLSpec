/**
 * MLSpec CLI Command Registration
 *
...
export function registerMlspecCommands(program: Command): void {
  ...
  // ml new <subcommand>
  const newCmd = program.command('new').description('Create new MLSpec entities');
...
`
        ...
        console.log(`2. Run 'mlspec add-evidence ${name} --level E1' to add evidence`);
      } catch (error) {
        ora().fail(`Error: ${(error as Error).message}`);
        process.exit(1);
      }
  ...
  // add-evidence <experiment> --level <level>
  program
...
`
  ...
  // promote <experiment> --to <candidate>
  program
    ...
    .action(async (experiment: string, options: { to: string }) => {
      try {
        ...
        if (!fs.existsSync(decisionPath)) {
          ora().fail(`decision.md not found for experiment '${experiment}'. Run 'mlspec decide ${experiment}' first.`);
          process.exit(1);
        }
      ...
      try {
        ...
        for (const experiment of listExperiments(projectPath)) {
            ...
            try {
              ...
              if (!result.success) {
                ...
                if (!refResult.valid) {
                  ...
                  warnings.push(`Experiment '${experiment}': missing comparison_ref`);
                }
              }
            } catch {
          ...
          if (hasEvidence && !fs.existsSync(hypothesisPath)) {
            errors.push(`Experiment '${experiment}' has evidence but no hypothesis.md`);
          }
